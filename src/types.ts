...
export const LumberCostSchema = z.object({
    ...
    wood_replace_id: z.number().nonnegative('Wood Replacement ID'),
    quantity: z.number().nonnegative('Quantity must be positive'),
    description: z.string().min(1, 'Job Description'),
    thickness: z.number().nonnegative('Thickness is required and must be positive'),
    length: z.number().nonnegative('Length is required and must be positive'),
